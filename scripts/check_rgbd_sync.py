...
def main():
    ...
    rclpy.init()
    node = rclpy.create_node('rgbd_sync_check', parameter_overrides=[
        Parameter('use_sim_time', value=True)])
    buffer = Buffer()
    listener = TransformListener(buffer, node)
    subscribers = [Subscriber(node, msg_type, topic, qos_profile=qos_profile_sensor_data)
                   for msg_type, topic in [(Image, '/rgbd/image'),
                       (Image, '/rgbd/depth_image'), (CameraInfo, '/rgbd/camera_info')]]
    ...
    def receive(rgb, depth, info):
        ...
        try:
            ...
            return
        stamps = [rclpy.time.Time.from_msg(msg.header.stamp).nanoseconds
                  for msg in (rgb, depth, info)]
        samples.append({'stamp_ns': stamp.nanoseconds,
            'spread_ms': (max(stamps) - min(stamps)) / 1e6,
            'rgb_size': [rgb.width, rgb.height],
            'depth_size': [depth.width, depth.height],
            'rgb_encoding': rgb.encoding, 'depth_encoding': depth.encoding,
            'fx': info.k[0], 'fy': info.k[4],
            'map_camera_position': [transform.transform.translation.x,
    ...
    try:
        while rclpy.ok() and len(samples) < args.samples and time.monotonic() < deadline:
    ...
    passed = len(samples) >= args.samples and all(
        sample['fx'] > 0 and sample['fy'] > 0
        and sample['rgb_size'] == sample['depth_size'] for sample in samples)
