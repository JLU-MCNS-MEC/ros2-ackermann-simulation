...
def _launch(context):
    ...
    return [IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(
            bringup, 'launch', 'nav2_waypoint_nav.launch.py')),
        launch_arguments={
            'localization_mode': mode, 'map_file': map_file,
...
def generate_launch_description() -> LaunchDescription:
    ...
    return LaunchDescription([
        DeclareLaunchArgument('mode', default_value='mapping', choices=['mapping', 'amcl']),
