...
- 更新：M0 已采集 21 次尝试（18 次至失败 + 审查后补充 3 次），20 成功、1 失败，
  6 次恢复；中位 26.77 s / P95 77.43 s。是包含失败的基线，不是稳定性通过。
- 原始点云 10 Hz、最大间隔 0.1 s，但导航扫描约 7.26 Hz、最大间隔 1 s；
  第 18 次触发安全断流停车，随后 TF 时间差导致终止。已将桥与两个投影节点
  组合进 `lidar_pipeline`，使用进程内通信，仍保留唯一 `/scan/points` 发布者。
- M1 首轮静止检查失败（120 s 匹配 274 组）；增加可选可靠接收模式后，
  新管线静止检查匹配 1,658 组，TF 有效 1,644 组，原质量门槛通过。
  新增数据年龄检查，正在进行运动检查和 5 目标导航；完成前不勾选 M1。
  当前主日志 `/tmp/ackermann_m1_retry_20260910_saZRl1/launch.log`，RGB-D 已开启。
  两包构建通过，145 项源码测试通过。下列 M0/M1“执行中”是阶段历史记录。
